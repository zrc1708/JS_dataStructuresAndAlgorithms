...
class MaxHeap {
    ...
    siftUp(index) {
        ...
        while (index > 0 && this.compareFn(this.heap[parent], this.heap[index]) == Compare.LESS_THAN) { // {2} 
    ...
    siftDown(index) {
        ...
        if (left < size && this.compareFn(this.heap[element], this.heap[left]) == Compare.LESS_THAN) { // {3} 
            element = left; // {4} 
        }
        if (right < size && this.compareFn(this.heap[element], this.heap[right]) == Compare.LESS_THAN) { // {5} 
            element = right; // {6} 
        }
        if (index !== element) { // {7} 
